...
> **⚠ 本文档的数字已作废（2026-09-19）。** 这里记录的 Phase-1 值 —— BC **43.2%** / ASAP **91.2%** / **+48.0 pp**（500 seeds）—— 产生于**旧几何 `U(30,60)`**，而且其中的 BC 一项与 CLR 一项来自**不同的几何代次**，本身就不能混引。论文采用的权威测量是 **BC 46.50% → SPC 90.75%（+44.25 pp，400 seeds，几何 `U(0,60)`，exact McNemar p = 1.04e-53）**，见 `paper/small_paper_draft.md` §6 台账与 `paper/aamas_paper.tex`。本文档**只作历史记录保留，不得用于引用**。
