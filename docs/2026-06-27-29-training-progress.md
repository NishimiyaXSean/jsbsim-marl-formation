...
| Phase 3.4 | Box(2)+hot | 4.9% | ~13% | 0.00 | 自我蒸馏, diff=0.4 热启动 |

### 最佳模型

| 维度 | 模型 | 路径 |
|------|------|------|
| **最高成功率** | Phase 2.2 | `phase2_continuous_0627_1551_s42_bc` (65.4% @ diff=0.26) |
| **最高难度** | Phase 3v3 | `phase2_continuous_0627_2326_s42_bc` (50.0% @ diff=0.58) |
...
### 4.1 Timeout 率仍高 (50% @ Phase 3v3)

在 diff=0.58 下，50% 的 episode 以 timeout 结束。timeout 的 mean min_dist ≈ 4259m——Agent 根本没能靠近目标。这不是 "搜索" 问题（Agent 有全向感知），而是 **OOD 初始几何**（bearing >25°）导致的分布外盲区。
