"""SinglePursuitTask PPO Training — Stage 2 (1v1 pursuit, hierarchical actions).

Usage:
    conda activate marl_env
    python scripts/train_single_pursuit_base.py --iterations 150 --difficulty 0.0
"""
...
def main():
    ...
    run_name = f"rllib_pursuit_{timestamp}_s{args.seed}"
    ...
    config = (
        ...
        .training(
            ...
            entropy_coeff=0.05, vf_clip_param=1000.0, grad_clip=0.5,
    ...
    for i in range(args.iterations):
        ...
        print(f"[iter {i:3d}] reward={reward:+.1f}  len={length:.0f}  fps={fps:.0f}")
    ...
    print(f"\nBest reward: {best_reward:.1f}  → {project_root}/checkpoints/best")
    ray.shutdown()
